...
static std::unique_ptr<PlanOperator> buildOperator(std::string& op) {
   toLower(op);
   if (op == "merge") {
      return std::move(std::unique_ptr<PlanOperator>(new MergeJoin()));
   } else if (op == "sort") {
      return std::move(std::unique_ptr<PlanOperator>(new Sort()));
   } else if (op == "select") {
      return std::move(std::unique_ptr<PlanOperator>(new Select()));
   } else if (op == "project") {
      return std::move(std::unique_ptr<PlanOperator>(new Project()));
   } else if (op == "scan") {
      return std::move(std::unique_ptr<PlanOperator>(new TableScan()));
   } 
   throw;
}
